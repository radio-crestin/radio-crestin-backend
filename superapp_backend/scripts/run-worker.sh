...
if [ "$DEBUG" = "true" ] || [ "$DEBUG" = "1" ] || [ "$DEBUG" = "t" ]; then
    ...
    watchmedo auto-restart --directory=./ --pattern=*.py --recursive -- celery -A superapp worker --loglevel=INFO --beat --scheduler django_celery_beat.schedulers:DatabaseScheduler --concurrency=5
else
