...
def generate_deployment():
    ...
    for station in get_stations():
        ...
        deployment["services"][f"hls_{station_slug}"] = {
            ...
            # with aac
            "command": f"bash -c \"ffmpeg -y -abort_on empty_output_stream -i '{station_stream}' -c:a libfdk_aac -profile:a aac_he -b:a 64k -flags +global_header -async 1 -ac 2 -ar 44100 -map 0:a:0 -bufsize 30000000 -f hls -hls_init_time 7 -hls_time 30 -hls_list_size 10 -hls_delete_threshold 30 -hls_flags delete_segments+omit_endlist -hls_start_number_source epoch -hls_segment_filename /data/%d.ts -master_pl_publish_rate 1 -sc_threshold 0 /data/index.m3u8 || (rm -rf /data/* && kill 1)\"",
