...
def generate_deployment():
    ...
    for station in get_stations():
        ...
        deployment["services"][f"hls_{station_slug}"] = {
            ...
            "restart": "always",
            "command": f"ffmpeg -y -i '{station_stream}' -c:a:0 libfdk_aac -profile:a:0 aac_ld -b:a:0 64k -c:a:1 copy -async 1 -ac 2 -r 44100 -map 0:a:0 -map 0:a:0 -f hls -hls_init_time 2 -hls_time 6 -hls_list_size 5 -hls_delete_threshold 10 -master_pl_name index.m3u8 -var_stream_map 'a:0,name:normal,default:yes a:1,name:high' -hls_flags delete_segments+omit_endlist -hls_start_number_source epoch -master_pl_publish_rate 2 -sc_threshold 0 /data/%v/index.m3u8"
        }
    return deployment
