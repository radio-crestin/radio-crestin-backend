...
class ListeningEvents(models.Model):
    """
    Legacy model - will be migrated to ListeningSessions.
    ...
    class Meta:
        managed = True
        verbose_name = _("Listening Event (Legacy)")
        verbose_name_plural = _("Listening Events (Legacy)")
        db_table = 'listening_events'
        ordering = ('-created_at',)

    def __str__(self):
        return f"Legacy: {self.user} - {self.station.title} - {self.start_time}"
